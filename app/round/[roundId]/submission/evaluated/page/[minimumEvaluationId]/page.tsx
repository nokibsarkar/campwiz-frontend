...
type AlreadyEvaluatedPaginationProps = {
    params: Promise<{
...
const AlreadyEvaluatedPagination = async ({ params }: AlreadyEvaluatedPaginationProps) => {
    ...
    const evaluationResponse = await loadNextEvaluation({ roundId: round.roundId, limit: limit, includeSubmissions: true, isPublic: false, includeEvaluated: true, includeNonEvaluated: false, next: minimumEvaluationId });
    ...
    return (
        <Suspense fallback={<LinearProgress />}>
            <Header returnTo={`/campaign/${round.campaignId}`} />
            <ActualPage initialEvaluations={evaluationResponse.data} next={evaluationResponse.next} round={round} prev={evaluationResponse.prev} />
        </Suspense >
    )
}
export default AlreadyEvaluatedPagination
...
import "./index.css"
import { VotingInterfaceProps } from "./VotingInterface";
...
const RankingVotingInterface = ({ }: VotingInterfaceProps) => {
    ...
    return (
        <div>
            ...
            >
                {items.map(({ name, image }, position) => (
                    <SortableItem key={name + position}>
                        <Box sx={classes.item}>
                            <Avatar
                                sx={classes.image}
                                alt={name}
